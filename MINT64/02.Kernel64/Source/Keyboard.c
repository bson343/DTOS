...
// 출력 버퍼(포트 0x60)[키보드 입장에서 출력]에 수신 된 데이터가 있는지 여부를 반환
BOOL kIsOutputBufferFull( void )
{
    // 상태 레지스터(포트 0x64)에서 읽은 값에
    // 출력 버퍼 상태 비트(비트 0)가 1로 설정되어 있으면 출력 버퍼에 키보드가 전송한 데이터가 존재함
    if ( kInPortByte( 0x64 ) & 0x01 ) 
    {
        return TRUE; // 요약: cpu가 키보드에서 읽을 것이 *있다*
    }
    return FALSE; // 요약: cpu가 키보드에서 읽을 것이 *없다*
}

// 입력 버퍼(포트 0x60)[키보드 입장에서 입력]에 프로세서가 쓴 데이터가 남아있는지 여부를 반환
BOOL kIsInputBufferFull( void )
{
    ...
    {
        return TRUE; // 요약: cpu가 키보드에 내린 명령이 처리 *되었다*
    }
    return FALSE; // 요약: cpu가 키보드에 내링 명령이 처리 *되지 않았다*
}
...
BOOL kActivateKeyboard( void )
{
    ...
    // 컨트롤 레지스터(포트 0x64)에 키보드 컨트롤러 활성화(0xAE)를 전달하여 키보드 디바이스 활성화
    kOutPortByte( 0x64, 0xAE ); // kOutPortByte( 컨트롤 레지스터의 포트 번호, 키보드 디바이스 활성화 커맨드)

    // 입력 버퍼(포트 0x60)[키보드 컨트롤러 입장에서 입력]가 빌 때까지 기다렸다가 키보드에 활성화 커맨드를 전송
    // 0xFFFF(65,535)만큼 루프를 수행할 시간이면 충분히 커맨드가 전송될 수 있음
    // 0xFFFF 루프를 수행한 이후에도 입력 버퍼(포트 0x60)[키보드 입장에서 입력]가 비지 않으면 무시하고 전송
    for ( i = 0; i < 0xFFFF; i++)
    {
        // 입력 버퍼(0x60)가 비어있으면 키보드 커맨드 전송 가능
    ...
    // 입력 버퍼(포트 0x60)[키보드 입장에서 입력]로 키보드 활성화(0xF4) 커맨드를 전달하여 키보드 전송
    kOutPortByte(0x60, 0xF4); // kOutPortByte( 컨트롤 레지스터의 포트 번호, 키보드 디바이스 활성화 커맨드)

    // ACK(0xFA)가 올 때까지 대기함
    // ACK가 오기 전에 키보드 풀력 버퍼(포트 0x60)에 키 데이터가 저장도리 수 있으므로
    // 키보드에서 전달된 데이터를 최대 100개까지 수신하여 ACK를 확인
    for ( j = 0; j < 100; j++)
    {
        // 0xFFFF(65,535)만큼 루프를 수행할 시간이면 충분히 커맨드가 전송될 수 있음
        // 0xFFFF 루프를 수행한 이후에도 출력 버퍼(포트 0x60)[키보드 입장에서 출력]가 비지 않으면 무시하고 읽음
        for ( i = 0; i < 0xFFFF; i++ )
        {
            // 출력 버퍼(포트 0x60)[키보드 입장에서 출력]가 차있으면 데이터를 읽을 수 있음
        ...
        // 출력 버퍼(포트 0x60)[키보드 입장에서 출력]에서 읽은 데이터가 ACK(0xFA)이면 성공
...
BOOL kChangeKeyboardLED( BOOL bCapsLockOn, BOOL bNumLockOn, BOOL bScrollLockOn)
{
    ...
    //키보드가 LED 상태 변경 명령을 가져갔으므로 ACK가 올때까지 대기
    for (j = 0; j < 100; j++)
    {
        for (i = 0; i< 0xFFFF; i++)
        {
...
BOOL kIsUseCombinedCode( BOOL bScanCode )
{
    ...
    {
        if (gs_stKeyboardManager.bScrollLockOn ^ gs_stKeyboardManager.bCapsLockOn)
        {
    ...
    {
        //NumLock 키가 눌려져있으면, 조합된 키를 되돌려 줌
...
void UpdateCombinationKeyStatusAndLED( BYTE bScanCode )
{
    ...
    // 눌림 또는 떨어짐 상태 처리, 최상위 비트 (비트 7)가 1이면 키가 떨어졌음을 의미하고
    // 0이면 눌렸음을 의미
    if ( bScanCode & 0x80)
    {
        bDown = FALSE;
    ...
    // CapsLock 키의 스캔 코드(58)이면 CapsLock의 상태 갱신하고 LED 상태 변경
