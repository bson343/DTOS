...
; 보호 모드 진입 절차
    ...
    mov eax, 0x4000003B             ; PG=0, CD=1, NW=0, AM=0, WP=0, NE=1, ET=1, TS=1, EM=0, MP=1, PE=1
                                    ; (부정확)모드에 따라 메모리 접근 제한과 레지스터 용도 차이는 있지만,
                                    ; (부정확)모드 관계 없이 설계 된 레지스터 범위 전부 접근이 가능한 것 같다.
                                    ; (부정확)리얼모드에서 eax범위의 레지스터를 사용하는 것을 보니
                                    ; 분명 [그림 3-4] 리얼모드에서는 32비트 레지스터에 접근 할 수 없다고 되어있다. 이상하네
                                    ; [표 3-3]에서는 리얼모드에서 32bit 피연산자를 받을 수 있다고 되어 있다.
                                    ; 책에는 명시적인 언급이 없지만 위키에 따르면 opcode에 prefix로 66이 부여되어 있으면 리얼모드에서 32bit 피연산자를 다룰 수 있다고 한다. [https://wiki.osdev.org/X86-64_Instruction_Encoding#:~:text=is%20not%20known.-,Operand,-%2Dsize%20and%20address]
...
GDTR:
    ...
    dd ( GDT - $$ + 0x10000)                ; 아래에 위치하는 GDT 테이블의 시작 주소
